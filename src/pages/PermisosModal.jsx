  ...
  const fetchAllAndAssignedPermissions = async () => {
    ...
    try {
      ...
      const assignedResponse = await fetch(
        `${API_BASE_URL}/UsuarioPermiso/${user.idUsuario}`
      );
      let assignedData = [];
  ...
  const movePermission = (permission, from, to) => {
    if (from === "available" && to === "assigned") {
      setAvailablePermissions((prev) =>
        prev.filter((p) => p.idPermiso !== permission.idPermiso)
      );
      setAssignedPermissions((prev) =>
        [...prev, permission].sort((a, b) =>
          a.nombrePermiso.localeCompare(b.nombrePermiso)
        )
      );
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/UsuarioPermiso/update-for-user/${user.idUsuario}`,
        {
          method: "PUT",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          data.message ||
            `Error ${response.status}: Fallo al actualizar permisos.`
        );
      }

      Swal.fire(
        "Éxito",
        `Permisos actualizados para ${user.nombre}.`,
        "success"
      );
      ...
      Swal.fire(
        "Error",
        error.message || "Fallo de conexión al guardar permisos.",
        "error"
      );
    } finally {
      setLoading(false);
